$(function(){
	...
	setTimeout(function(){
		...
		var weaponSkillMap = {
			...
			"屬性角色自身所剩HP越少攻擊威力越大":["pride", 0, 0.5, 19],
			"屬性角色所剩的HP愈少攻擊威力愈大(小)":["pride", 0, 0.35, 11.4],
			"屬性角色所剩的HP愈少攻擊威力愈大(中)":["pride", 0, 0.5, 11.4],
			"屬性角色所剩的HP愈少攻擊威力愈大(大)":["pride", 0, 0.5, 19],
			...
			"屬性角色的弱點攻擊機率上升(小)":["stinger", 0, 0],
			"屬性角色的弱點攻擊機率上升(中)":["stinger", 1, 0],
			"屬性角色的弱點攻擊機率上升(大)":["stinger", 2, 0],
			...
			//属性キャラクターの残存HP割合が
			"HP比例愈多攻撃力上升(小)":["vigorous", 8, 0.2],
			"HP比例愈多攻撃力上升(中)":["vigorous", 12, 0.2],
			"HP比例愈多攻撃力上升(大)":["vigorous", 16, 0.2],
		...
		//繁中用前缀属性复制
       WEAPON_SKILL_MAP_PREF["刃身"] = WEAPON_SKILL_MAP_PREF["ブレイド"]; //"剑",
		WEAPON_SKILL_MAP_PREF["撕裂"] = WEAPON_SKILL_MAP_PREF["リッパー"]; //"特殊剑",
		WEAPON_SKILL_MAP_PREF["長槍"] = WEAPON_SKILL_MAP_PREF["サリッサ"]; //"枪",
		WEAPON_SKILL_MAP_PREF["健斧"] = WEAPON_SKILL_MAP_PREF["アックス"]; //"斧",
		WEAPON_SKILL_MAP_PREF["神杖"] = WEAPON_SKILL_MAP_PREF["スタッフ"]; //"杖",
		WEAPON_SKILL_MAP_PREF["重錘"] = WEAPON_SKILL_MAP_PREF["マレット"]; //"锤",
		WEAPON_SKILL_MAP_PREF["加農"] = WEAPON_SKILL_MAP_PREF["キャノン"]; //"铳",
		WEAPON_SKILL_MAP_PREF["弓箭"] = WEAPON_SKILL_MAP_PREF["クォレル"]; //"弓",
        WEAPON_SKILL_MAP_PREF["聖杯"] = WEAPON_SKILL_MAP_PREF["チャリス"]; //"魔导具"
		...
		//繁中用后缀属性复制
		WEAPON_SKILL_MAP_SUF["電子連擊"] = WEAPON_SKILL_MAP_SUF["アバランシェ"]; //"剑",
		WEAPON_SKILL_MAP_SUF["三爪痕"] = WEAPON_SKILL_MAP_SUF["トライエッジ"]; //"特殊剑",
		//WEAPON_SKILL_MAP_SUF["剛毅"] = WEAPON_SKILL_MAP_SUF["ストレングス"]; //"枪",待修正
		WEAPON_SKILL_MAP_SUF["強健"] = WEAPON_SKILL_MAP_SUF["ストレングス"];
		WEAPON_SKILL_MAP_SUF["戰略"] = WEAPON_SKILL_MAP_SUF["タクティクス"]; //"斧",
		WEAPON_SKILL_MAP_SUF["恩寵"] = WEAPON_SKILL_MAP_SUF["グレイス"]; //"杖",
		WEAPON_SKILL_MAP_SUF["虐殺"] = WEAPON_SKILL_MAP_SUF["カーネイジ"]; //"锤",
		WEAPON_SKILL_MAP_SUF["支援"] = WEAPON_SKILL_MAP_SUF["プロテクト"]; //"铳",
		WEAPON_SKILL_MAP_SUF["瞄準"] = WEAPON_SKILL_MAP_SUF["スラッグ"]; //"弓",
        WEAPON_SKILL_MAP_SUF["城壁"] = WEAPON_SKILL_MAP_SUF["ランパート"]; //"魔导具"
		WEAPON_SKILL_MAP_SUF["強化"] = WEAPON_SKILL_MAP_SUF["エンハンス"]; //"装備中の「XXX」の攻撃・HPステータスUP"
		...
		function createEnhanceRCupBonusBtn() {
			var enhanceRCupBonusBtn = $("<button type='button' class='btn'>强化R圣杯到lv3++</button>");
		...
		function createEnhanceSRWeapon3Btn() {
			var enhanceSRWeapon3Btn = $("<button type='button' class='btn'>强化SR武器到lv3(不支持)</button>");
		...
		function createRefreshExistBtn() {
			...
			refreshExistBtn.click(function(){
				...
				function createDoQuestBtn() {
		var doQuestBtn = $("<button type='button' class='btn'>捐金币</button>");
		doQuestBtn.click(function(){
			emptyLog();
			doQuestBtn.prop("disabled",true);
			var missionApi = kh.createInstance("apiAMissions");
			missionApi.getDaily().then(function(daily){
				var dRet = daily.body;
				var pro = Promise.resolve();
				if(dRet.complete){
				}else{
					_.each(dRet.missions,function(item,i){
						if(!item.clear){
							if(item.description == "完成1次公會捐獻"){
								pro.then(function(){
									mypageLog("开始捐钱");
									return kh.createInstance("apiAUnions").donation(1);	
								});	
							}else if(item.description == "向其他玩家發送1次訊息"){
								pro.then(function(){
									var apiAFriends = kh.createInstance("apiAFriends");
									mypageLog("开始留言,获取好友列表");
									return apiAFriends.getList("all",1,4).then(function(data){
										if(data.body.data.length>0){
											var playerId = data.body.data[0].a_player_id;
											return _http.post({
								                url: "/v1/a_greets",
								                json: {
								                    to_a_player_id: playerId,
								                    stamp_id: 2
								                }
								            });
										}else{
				...
				pro.then(function(){
					mypageLog("执行完毕");
					doQuestBtn.prop("disabled",false);
				});
			});
		});
		secondLevelMenuDiv.append(doQuestBtn);
		}
		//检查raid
		...
		function showInfo() {
			emptyLog();
			mypageLog("修改：bluesword");
			mypageLog("更新日期：2019/6/13");
			mypageLog("暗号：650929703，繁中欢迎你");
			
			mypageLog("由于后缀一致，单旺盛现在无法统计，待运营修复");
			/*mypageLog("更新日志：");
